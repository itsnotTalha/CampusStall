"use client";

import { useRouter } from "next/navigation";
import { useState, useTransition } from "react";
import { Bookmark } from "lucide-react";

import { setProjectSavedAction } from "@/app/(dashboard)/saved/actions";
import { useRequireAuth } from "@/hooks/use-require-auth";
import { cn } from "@/lib/utils";

type SaveButtonProps = {
  listingTitle: string;
  className?: string;
  initialSaved?: boolean;
  projectId?: string;
  showLabel?: boolean;
};

export function SaveButton({
  listingTitle,
  className,
  initialSaved = false,
  projectId,
  showLabel = false,
}: SaveButtonProps) {
  const [saved, setSaved] = useState(initialSaved);
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();
  const router = useRouter();
  const { checkingSession, requireAuth } = useRequireAuth();

  async function handleSave() {
    if (!(await requireAuth())) return;

    const nextSaved = !saved;
    setError(null);

    if (!projectId) {
      setSaved(nextSaved);
      return;
    }

    setSaved(nextSaved);
    startTransition(async () => {
      const result = await setProjectSavedAction(projectId, nextSaved);

      if (result.error) {
        setSaved(!nextSaved);
        setError(result.error);
        return;
      }

      router.refresh();
    });
  }

  return (
    <>
      <button
        aria-label={`${saved ? "Remove" : "Save"} ${listingTitle}`}
        aria-pressed={saved}
        className={cn(
          "flex size-8 items-center justify-center rounded-lg border bg-card/95 text-muted-foreground shadow-sm outline-none transition-colors hover:text-primary focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-60",
          showLabel && "h-10 w-full gap-2 px-3 text-sm font-medium",
          saved && "border-primary/20 bg-primary text-primary-foreground",
          className,
        )}
        disabled={checkingSession || pending}
        onClick={handleSave}
        title={error ?? undefined}
        type="button"
      >
        <Bookmark
          aria-hidden="true"
          className={cn("size-4", saved && "fill-current")}
        />
        {showLabel && <span>{pending ? "Saving…" : saved ? "Saved" : "Save"}</span>}
      </button>
      {error && (
        <span className="sr-only" role="alert">
          {error}
        </span>
      )}
    </>
  );
}
